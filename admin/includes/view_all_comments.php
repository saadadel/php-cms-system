<?php
    if(isset($_GET['delete'])  && isset($_SESSION['user_role']) && $_SESSION['user_role'] == "admin")
    {
        ...
        header("location: comments?source=view_all_comments");
...
<table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                <?php
                                    ...
                                    {
                                        $post_q = "select * from posts where post_id = {$row['comment_post_id']}";
                                        ...
                                        echo "<tr>";
                                            ...
                                            echo "<td><a href='../post?post_id={$row['comment_post_id']}'>{$post['post_title']}</a></td>";
                                            ...
                                            {
                                                echo "<td>Approved</td>";
                                                echo "<td><a href='comments?source=view_all_comments&status=0&comment_id={$row['comment_id']}'>Unapprove</a></td>";
                                            }
                                            elseif($row['comment_status'] == 0)
                                            {
                                                echo "<td>Unapproved</td>";
                                                echo "<td><a href='comments?source=view_all_comments&status=1&comment_id={$row['comment_id']}'>Approve</a></td>";
                                            }
                                            echo "<td><a onClick=\"javascript: return confirm('Arer you sure you want to DELETE?')\" href='comments?source=view_all_comments&delete={$row['comment_id']}&post={$row['comment_post_id']}'>Delete</a></td>";
                                        echo "</tr>";
                                    }
                                ?>
                            </tbody>
